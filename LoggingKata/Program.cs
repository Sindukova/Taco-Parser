...
namespace LoggingKata
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (lines.Count() == 0)
            {
                logger.LogError("0 lines read from CVS file");
            }
            ...
            for (int i = 0; i < locations.Length; i++)
            {
                ...
                for (int x = 0; x < locations.Length; x++)
                {
                    ...
                    if (corC.GetDistanceTo(corD) < distanceS && corC.GetDistanceTo(corD) != 0)
                    {
                        distanceS = corC.GetDistanceTo(corD);
                        tacoBellC = locC;
                        tacoBellD = locD;
                    }
                }
            }
            Console.WriteLine($"{tacoBellC.Name} and {tacoBellD.Name} are the closest.");
